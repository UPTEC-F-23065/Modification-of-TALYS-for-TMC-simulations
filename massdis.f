...
c	Added if condition to check if user supplied a value to the 
c	common variable "yieldfileid". 
c	If no:  do nothing
c	if yes: Add string "yieldfileid" to GEF FY file name in order to 
c		read specific yieldfile when multithreading.
c *************************************************************************
...
c
  ...
  154       close (1)
            ...
            do k=1,Nfftab
              ...
              Estring(5:5)='e'
	      if (yieldfileid == 'novalue') then
		Yfile(k)=trim(ffpath)//trim(nucstring)//'_'//Estring//
     +          'MeV_'//trim(ffname)//'.ff'
		print *, 'YIELDFILE_ID IS ', yieldfileid
	      else
              	Yfile(k)=trim(ffpath)//trim(nucstring)//'_'//Estring//
     +          'MeV_'//trim(ffname)//'_'//trim(yieldfileid)//'.ff'
	      endif  	      
            enddo
          else
            Nfftab=1
